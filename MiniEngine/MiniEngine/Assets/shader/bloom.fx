...
float4 PSSamplingLuminance(PSInput In) : SV_TARGET0;
{
    ...
    float2 tex = In.uv;
    tex += float2(-0.5f/texSize.x,-0.5f/texSize.y);
    falot4 color = sceneTexture.Sample(Sampler,tex);
...
float4 PSBlur(PS_BlurInput In):SV_TARGET0
{
    ...
    Color += weight[1].x * (luminanceTexture.Sample(Sampler,In.tex5)
                + luminanceTexture.Sample(Sampler,In.tex2 + offset));
    Color += weight[1].x * (luminanceTexture.Sample(Sampler,In.tex6)
                + luminanceTexture.Sample(Sampler,In.tex1 + offset));
    Color += weight[1].x * (luminanceTexture.Sample(Sampler,In.tex7)
